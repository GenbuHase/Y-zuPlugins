...
class Bouyomi {
	...
	speak (speed, pitch, volume, type, text) {
		const socket = this.socket = new WebSocket(`ws://localhost:${this.port}`);
...
const bouyomi = new Bouyomi(50002);
...
document.querySelector('Div[Role="feed"]').addEventListener("DOMNodeInserted", event => {
	...
	if (post.nodeName === "ARTICLE" && post.parentNode.nodeName !== "HEAD") {
		const content = post.querySelector("Div.status__content");
		if (!content) return;
